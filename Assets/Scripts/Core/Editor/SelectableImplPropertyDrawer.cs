...
namespace Core
{
    ...
    public class SelectableImplPropertyDrawer : PropertyDrawer
    {
        private static readonly Dictionary<string, Type[]> TypeCache = new();

        /// <summary>
        ///    extracts object from the property.
        ///    we start from the root object (prop.serializedObject.targetObject)
        ///    then we take property path (prop.propertyPath) and parse it.
        ///    We split it by dots and indexes,
        ///    then we use reflection to get each field/indexer in the path one by one,
        ///    descending into the object tree until we reach the target object.
        /// </summary>
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (newIndex != index) CreateInstance(property, newIndex, types);
